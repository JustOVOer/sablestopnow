...
| 文件 | 说明 |
|---|---|
| `icon-512-dark.png` | **成品 1:1（深灰底 + 浅灰网格）**，512×512 |
| `icon-512-lightblue.png` | **成品 1:1（浅蓝底 + 网格）**，512×512 |
| `icon-16x10-dark.png` | **成品 16:10（深灰底 + 网格）**，1024×640 |
| `icon-16x10-lightblue.png` | **成品 16:10（浅蓝底 + 网格）**，1024×640 |
...
# 3) 渲染四个成品
#    参数：<ss> <底色> <网格色> <步长> <光边色> <光边alpha> <提亮gamma> <宽> <高>
java branding/tools/IconRenderer.java branding/scene.txt branding/textures branding/icon-512-dark.png          2 33383D 4A5158 32 9FB6CC 0.55 1.6 512  512
java branding/tools/IconRenderer.java branding/scene.txt branding/textures branding/icon-512-lightblue.png     2 A9CBE8 8FB6D8 32 53718C 0.45 1.6 512  512
java branding/tools/IconRenderer.java branding/scene.txt branding/textures branding/icon-16x10-dark.png        2 33383D 4A5158 32 9FB6CC 0.55 1.6 1024 640
java branding/tools/IconRenderer.java branding/scene.txt branding/textures branding/icon-16x10-lightblue.png   2 A9CBE8 8FB6D8 32 53718C 0.45 1.6 1024 640
```

> 画布尺寸参数省略时用 `scene.txt` 里的正方形尺寸。非正方形画布走**等比 contain**：整体缩放到画布内并居中，不会裁掉手杖两端。
...
- **齿轮**：`R_x(-55°)` 立起来 + `R_y(25°)` 转向右上，scale 0.88（按左下角空三角的**内切圆**定尺：半径约 7.9 单位、中心落在屏幕坐标 (-7.4, -7.4)，刚好填满又不与手杖相碰）；深度放 **-3**（比手杖更远），所以重叠处是**手杖压在齿轮前面**；另加一次**屏幕空间上下镜像**（`flip_screen_y`，以该物件的投影中心为轴翻 sy）。
