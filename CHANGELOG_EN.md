...
- **New control scheme** (`new_control_scheme`, on by default, overrides the old bindings): **Ctrl** toggles multi-select, the **wheel** switches function, **LMB** applies the selected function; the old Z/V/O/K/R/C/X now exist only as function entries. Three modes (normal / multi-select / group drag) each have their own function list and remember the last selection.
  ...
  - **Idle state**: switching to the staff does **not** enter any mode, and **every staff input except "LMB to enable" and the wheel (hotbar) is ignored** — RMB/MMB, Aeronautics' native right-click drag and Ctrl multi-select all do nothing — while the HUD says "LMB to enable". One LMB enables it; switching away resets it.
  ...
  - **Lock is the first function in both normal and group-drag mode** (matching the original left-click-to-lock), so the cursor starts on it after enabling.
- **A single body can enter drag mode too**: right-clicking a body that is not in the queue builds a **temporary one-member group** and enters group drag. That temporary group **coexists with the main multi-select queue**, is dropped when the drag ends and is never outlined in blue.
- **Functions target the body (or group) you are looking at**: lock / no-collision / snapshot / scale / ownership resolve the crosshair first — a group member means the whole group, a body outside the group means just that body, and only when nothing is aimed at do they fall back to the current group (previously a non-empty queue always won).
- **Mod icon**: `src/main/resources/icon.png` plus `logoFile` in `mods.toml`, shown in NeoForge's mod list.
...
- **Clear queue / Shift+RMB could not cancel the selection**: `releaseAllClaims()` returned early once the local claim flag had been reset (the server claim was not necessarily gone), and "clear queue" only emptied the main queue, leaving the temporary drag group behind. Clearing now notifies the server unconditionally and drops the temporary group too.
